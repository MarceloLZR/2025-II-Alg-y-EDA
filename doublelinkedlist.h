...
void CDoubleLinkedList<Traits>::InternalInsert(Node *&rParent, value_type &elem, Ref ref){
    // Si la lista está vacía, el nuevo nodo será raíz y cola
    if(!rParent){
    ...
    if(!rParent->GetNext()){
        Node *pNew = new Node(elem, ref, nullptr);
        rParent->SetNext(pNew);
        pNew->SetPrev(rParent);
        m_pTail = pNew;
        m_nElem++;
        return;
    }
...
CDoubleLinkedList<Traits>::CDoubleLinkedList(CDoubleLinkedList &other){
    for(auto it = other.begin(); it != other.end(); ++it){
        value_type elem = *it;
        Ref ref = 0; // puedes ajustar según tus traits
        Insert(elem, ref);
    }
}
